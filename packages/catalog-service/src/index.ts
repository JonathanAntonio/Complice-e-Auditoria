...
import { loadCatalogServiceConfig } from "./app/config";

const config = loadCatalogServiceConfig(process.env);
...
async function bootstrap() {
  ...
  app.listen(config.port, () => {
    logger.info(`Catalog service listening on http://localhost:${config.port}`);
  });
