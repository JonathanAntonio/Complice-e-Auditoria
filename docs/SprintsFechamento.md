...
| 2026-05-24 | Sprint 4 | RN-056 integrado ponta a ponta no frontend: BFF estendido com `GET/PUT /notifications/preferences/:recipient`, client/frontend atualizado com seção “Preferências por destinatário (RN-056)” na tela de Notificações; validação MCP Playwright autenticada concluída com persistência real (`PUT 200` + `GET 200` para `rn056.user@example.com`, `channels=[email]`, `frequency=hourly_digest`, `grouping=true`, `muteLowMedium=true`) | Realizar commit de fechamento do bloco RN-056 frontend e manter trilha operacional mensal | Developers |
