...
describe("UserController — cenários absurdos", () => {
  ...
  describe("getById", () => {
    it("não explode quando req.params.id é undefined", async () => {
      const controller = new UserController(createUserUseCase, getUserByIdUseCase, assignUserRoleUseCase);
  ...
  describe("assignRole", () => {
    it("não explode quando id é inválido", async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(400);
      expect(vi.mocked(assignUserRoleUseCase.execute)).not.toHaveBeenCalled();
    });
  });
});
