...
export async function requestBffNotification(path, options = {}) {
  const {
    method = "GET",
    body,
    defaultErrorMessage = "Falha na comunicação com o BFF (notification).",
  } = options;

  const hasBody = body !== undefined;
  const response = await fetch(`/bff/notifications${path}`, {
    method,
    headers: hasBody
