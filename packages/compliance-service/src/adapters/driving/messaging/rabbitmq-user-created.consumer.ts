...
import {
  ...
  QUEUE_USER_CREATED_COMPLIANCE,
  QUEUE_USER_CREATED_COMPLIANCE_FAILED,
 ...
 * Após MAX_RETRIES falhas, a mensagem é enviada para QUEUE_USER_CREATED_COMPLIANCE_FAILED
 ...
 * Após MAX_RETRIES, envia a mensagem para QUEUE_USER_CREATED_COMPLIANCE_FAILED e faz nack sem requeue.
 */
export class RabbitMqUserCreatedConsumer {
  ...
  async start(): Promise<void> {
    ...
    await this.channel.assertQueue(QUEUE_USER_CREATED_COMPLIANCE, { durable: true });
    await this.channel.bindQueue(
      QUEUE_USER_CREATED_COMPLIANCE,
      EXCHANGE_USER_EVENTS,
      "user_created"
    );
    await this.channel.assertQueue(QUEUE_USER_CREATED_COMPLIANCE_FAILED, { durable: true });

    await this.channel.consume(QUEUE_USER_CREATED_COMPLIANCE, async (msg: ConsumeMessage | null) => {
      ...
      try {
        ...
        if (count >= MAX_RETRIES) {
          ...
          this.channel.sendToQueue(QUEUE_USER_CREATED_COMPLIANCE_FAILED, msg.content, { headers });
